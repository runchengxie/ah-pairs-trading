...
这是一个面向同一发行人 A/H 标的的相对价值研究框架。默认执行模式是 `long_cheaper_leg_only`，适合不能做空但可以买入便宜腿的账户；`paired` 仅作为双腿研究模式保留，需要可做空环境。

## 适用与边界

- 适合做同发行人 A/H 相对价值研究、信号筛选和成本敏感型回测。
- 默认模式是 `long_cheaper_leg_only`，它不是市场中性策略，而是把相对价值信号落地成单腿买入。
- 如果你的账户既不能做空，也不能买入 H 股，那么默认模式也只能部分执行。
- 如果你要双腿多空，请显式切到 `--execution-mode paired`；该模式会做空一条腿。
