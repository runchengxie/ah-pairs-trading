...
这是一个面向同一发行人 A/H 标的的相对价值研究框架。默认执行模式是 `long_cheaper_leg_only`，把相对价值信号转成可执行的单腿买入决策；`paired` 保留给允许做空环境下的双腿研究。

## 项目定位

- 适合做同发行人 A/H 相对价值研究、信号筛选和成本敏感型回测。
- 默认模式是 `long_cheaper_leg_only`，不是默认市场中性统计套利模板。
- 如果需要双腿多空，请显式切到 `--execution-mode paired`；该模式会真实做空其中一条腿。
